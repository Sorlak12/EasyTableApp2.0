...
app.get("/detallesmesa/:idMesa", (req, res) => {
    ...
    db.query(`SELECT
        ...
        JOIN Comensal_Producto CP ON C.IDComensal = CP.IDComensal
        JOIN Producto P ON CP.IDProducto = P.IDProducto
        LEFT JOIN Comensal_Producto_Extra CPE ON CP.IDComensal = CPE.IDComensal AND CP.IDProducto = CPE.IDProducto
...
app.post("/agregarProducto/:idComensal/:idProducto/:cantidad/:entregado/:notas", (req, res) => {
    ...
    db.query(
        ...
        (err, results) => {
            ...
            db.query(
                `INSERT INTO comensal_producto (IDComensal, IDProducto, Cantidad, Entregado, Notas)
                 VALUES (?, ?, ?, ?, ?)
                 ON DUPLICATE KEY UPDATE Cantidad = Cantidad + VALUES(Cantidad)`,
                [idComensal, idProducto, cantidad, entregado, notaFinal],
                (err, results) => {
                    if (err) {
                        console.error("❌ Error al insertar producto:", err);
                        return res.status(500).json({ error: err.message });
                    }
                    console.log("✅ Producto insertado correctamente con nota:", notaFinal);
                    res.json({ message: "Producto insertado", nota: notaFinal });
                }
            );
        }
    );
...
//INSERT extra
app.post("/agregarExtra/:idComensal/:idProducto/:notas/:idExtra/:cantidad", (req, res) => {
    const idComensal = req.params.idComensal;
    const idProducto = req.params.idProducto;
    const notas = decodeURIComponent(req.params.notas);
    const idExtra = req.params.idExtra;
    const cantidad = parseInt(req.params.cantidad);


    // Primero, verificar que exista esa combinación exacta en comensal_producto
    const checkQuery = `
        SELECT * FROM comensal_producto
        WHERE IDComensal = ? AND IDProducto = ? AND Notas = ?
    `;

    db.query(checkQuery, [idComensal, idProducto, notas], (checkErr, rows) => {
        if (checkErr) {
        ...
        if (rows.length === 0) {
            console.warn("⚠️ No existe comensal_producto para los parámetros especificados:", { idComensal, idProducto, notas });
            return res.status(400).json({
                error: "No existe una instancia de comensal_producto con esa nota. No se puede agregar el extra."
            });
        }

        // Si pasa la validación, insertar el extra
        const insertQuery = `
            INSERT INTO comensal_producto_extra (IDComensal, IDProducto, Notas, IDExtra, Cantidad)
            VALUES (?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE Cantidad = Cantidad + VALUES(Cantidad)
        `;

        const insertParams = [idComensal, idProducto, notas, idExtra, cantidad];
...
app.delete("/eliminarProducto/:idProducto/:idComensal", (req, res) => {
    const idProducto = req.params.idProducto;
    const idComensal = req.params.idComensal;

    const deleteExtrasQuery = "DELETE FROM comensal_producto_extra WHERE IDComensal = ? AND IDProducto = ?";
    const deleteProductoQuery = "DELETE FROM comensal_producto WHERE IDComensal = ? AND IDProducto = ?";

    db.query(deleteExtrasQuery, [idComensal, idProducto], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });

        db.query(deleteProductoQuery, [idComensal, idProducto], (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
...
app.post('/actualizar_entregado/:id_comensal/:id_producto/:notas/:entregado', (req, res) => {
    const { id_comensal, id_producto, notas, entregado } = req.params;
    ...
    const selectQuery = `
        ...
        WHERE IDComensal = ? AND IDProducto = ? AND Notas = ?
    `;
    const selectParams = [id_comensal, id_producto, finalNotas];
    ...
    db.query(selectQuery, selectParams, (err, results) => {
        ...
        const updateQuery = `
            ...
            WHERE IDComensal = ? AND IDProducto = ? AND Notas = ?
        `;
        const updateParams = [entregadoBool, id_comensal, id_producto, finalNotas];
